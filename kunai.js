class Kunai extends Phaser.Physics.Arcade.Sprite {
    ...
    update() {
        ...
        if (!this.cooldown && this.alphaCD > 0) {
            this.alphaCD -= 0.02
            this.majCD();
        }
    ...
    majCD() { // à mettre dans la classe kunai après avoir refait l'UI
        this.showCD.clear();
        this.showCD.setScrollFactor(0).setDepth(1);
        this.showCD.fillStyle(0xff0000, this.alphaCD);
        this.showCD.slice(-250, 1150, 120, Phaser.Math.DegToRad(270), Phaser.Math.DegToRad(this.kunaiCD));
        this.showCD.setScrollFactor(0);
        this.showCD.fillPath();
    }
}
